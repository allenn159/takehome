...
function PersonDetails({ person }: { person: Person }) {
  const { data: homeworld, isLoading: homeworldLoading } = useSwapiResource<Planet>(person.homeworld);
  ...
  const speciesNames = speciesResults.map((r) => r.data?.name).filter(Boolean).join(", ") || "Human";
  const filmTitles = filmResults.map((r) => r.data?.title).filter(Boolean).join(", ") || "None";
  const vehicleNames = vehicleResults.map((r) => r.data?.name).filter(Boolean).join(", ") || "None";
  const starshipNames = starshipResults.map((r) => r.data?.name).filter(Boolean).join(", ") || "None";
  ...
  return (
    <Box mt={3} display="grid" gridTemplateColumns="1fr 1fr" gap={2}>
      ...
      <Text fontSize="sm"><Text as="span" fontWeight="medium">Homeworld:</Text> {homeworld?.name ?? "—"}</Text>
      <Text fontSize="sm" gridColumn="1 / -1"><Text as="span" fontWeight="medium">Species:</Text> {speciesNames}</Text>
      <Text fontSize="sm" gridColumn="1 / -1"><Text as="span" fontWeight="medium">Films:</Text> {filmTitles}</Text>
      <Text fontSize="sm" gridColumn="1 / -1"><Text as="span" fontWeight="medium">Vehicles:</Text> {vehicleNames}</Text>
      <Text fontSize="sm" gridColumn="1 / -1"><Text as="span" fontWeight="medium">Starships:</Text> {starshipNames}</Text>
    </Box>
  );
}
