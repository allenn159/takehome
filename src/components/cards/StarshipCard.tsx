...
function StarshipDetails({ starship }: { starship: Starship }) {
  ...
  const pilotNames = pilotResults.map((r) => r.data?.name).filter(Boolean).join(", ") || "None";
  ...
  return (
    <Box mt={3} display="grid" gridTemplateColumns="1fr 1fr" gap={2}>
      ...
      <Text fontSize="sm" gridColumn="1 / -1"><Text as="span" fontWeight="medium">Pilots:</Text> {pilotNames}</Text>
